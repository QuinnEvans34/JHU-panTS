...
Finding 3: the model over-predicts, and diagnosing why is the interesting part. My first real evaluation: pancreas Dice 0.72, lesion Dice 0.17 on tumor cases, but specificity only 8 percent. Naive cleanup (keep the largest blob, drop tiny ones) did nothing, because the false positives are large connected regions, not specks. The fix was geometric: a pancreatic lesion has to sit near the pancreas, and enforcing that lifted specificity from 8 percent to 42 percent with no retraining. A probability-threshold sweep barely helped, which tells me the leftover false positives are confident and near the organ, not low-confidence noise. Decision it forced: the Week 3 and 4 plan pairs a less trigger-happy retrain (balanced sampling) with that anatomical constraint. I show the overfit curve here as proof the pipeline learns, so the over-prediction is a tuning problem, not a broken pipeline.
...
- "Did post-processing help?" Size-based cleanup did not, because the false positives are large connected regions, not specks. The anatomical constraint did: requiring lesions to sit near the pancreas raised specificity from 8 to 42 percent. The cost was a small drop in lesion Dice on tumor cases (0.169 to 0.139), which is the sensitivity-for-specificity trade I expected.
