...
**Open / to personalize:** proposal Sections 2 (Why) & 3 (Takeaway) — Quinn's voice DONE. Still TODO this week: M2P2 presentation + `audience-notes-week2.md`; balanced retrain; Tue-Fri standups; commit + push.

## Next session
Draft M2P2 12-min presentation from the report's 3 key findings (imbalance → sampling+loss; geometry heterogeneity → resampling; over-prediction/8% spec → sens-spec strategy) + `audience-notes-week2.md` scaffold. Then kick off balanced retrain (`train.py`, 1:1 sampling, `--val-positive`) so Week 3 opens with fresh numbers. Commit Week 2 work (branch → merge).
