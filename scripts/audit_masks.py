...
def main():
    ...
    for i, r in enumerate(man.itertuples(), 1):
        ...
        bad = pv == 0 or pml < args.min_panc_ml
        rows.append({"case_id": r.case_id, "pancreas_mL": round(pml, 2),
                     "lesion_mL": round(lml, 2), "has_lesion": bool(getattr(r, "has_lesion", False)),
                     "flag": "TINY_OR_EMPTY_PANCREAS" if bad else ""})
        if bad:
    ...
    print(f"FLAGGED (empty/tiny pancreas): {len(flagged)} of {len(rep)}")
    for c in flagged[:20]:
        print("   ", c)
