...
5. 🖥️ Capture **MLflow screenshots** (the `pants-level45` tuning runs + the `pants-level45-optuna`
   experiment + the registered model) → `week4/img/`.
...
2. 🎨 Generate the **pipeline diagram** as an image → `week4/img/pipeline-diagram.png` (I'll produce it).
...
`week4/codex-passoff-code-plan.md`.
