...
- 🔲 **MLflow screenshots:** capture (a) the optuna experiment's tuning runs, (b) the registered model (Models tab) → save PNGs to `week4/img/`. *This is the only missing item for this criterion.*
...
- ✅ **Visualization:** by-size/by-phase bar chart (`week4/img/test-by-size-phase.svg`) + Week-3 diagrams.
- 🔲 **Sample predictions with confidence (CV track):** export 2–3 test-case overlays with the CADe confidence — `python scripts/export_case.py --ckpt <FINAL_CKPT> --split test --case <id> --whole-box --crop-native 16 --roi 128` → drop the PNGs in `week4/img/`. (Pick one small-miss + one good case for an honest contrast.)
...
- ✅ **Diagram:** `week4/img/pipeline-diagram.svg`.
...
1. 🔲 **Two MLflow screenshots** → `week4/img/` (optuna runs + registered model). [§1]
2. 🔲 **Endpoint smoke test** → paste `/health` + `/predict` output into report §4. [§4] *(GPU-free window)*
3. 🔲 **2–3 sample-prediction overlays** with confidence → `week4/img/`. [§2] *(GPU-free window)*
4. 🔲 **Git:** create a `week4/` branch, commit the `week4/` folder + the updated root `docs/`, merge to main.
