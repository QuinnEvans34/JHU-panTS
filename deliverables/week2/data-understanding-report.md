...
Author: Quinn. Week 2. Companion notebook: `deliverables/week2/eda-notebook.ipynb`.
...
Before spending real compute, the pipeline has to prove it can memorize a tiny fixed set. If the network cannot drive training loss down on a couple of cases, the data, loss, or labels are wired wrong. The figure is in the notebook (`deliverables/week2/overfit_curve.png`), pulled from the actual MLflow logs.
...
Manifest and splits: `python scripts/build_manifest.py` then `python scripts/create_splits.py`. EDA and figures: open `deliverables/week2/eda-notebook.ipynb`, which reads `outputs/manifest.csv` directly. The overfit figure is generated from the MLflow run database. No raw data is committed; the dataset path is set in `configs/level45.yaml`.
