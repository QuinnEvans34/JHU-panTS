...
**Diagram.** See `deliverables/week4/img/pipeline-diagram.svg` — the four stages (data prep → training → evaluation →
...
- Final eval → §2 + `scripts/evaluate.py` / `analyze_cases.py` on test + `outputs/test_percase.csv` + `deliverables/week4/img/test-by-size-phase.svg` + Week-3 diagrams.
- Orchestration → §3 + `deliverables/week4/img/pipeline-diagram.svg`.
