...
- At risk / still to do for Sunday: the one-time held-out test-set evaluation and its numbers, registering the final model in MLflow with screenshots, the pipeline diagram image, and writing the report itself plus these AI-documentation files. These are the remaining M4A1 items; none require new modeling, so the risk is time, not capability.
